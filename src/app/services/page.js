...
const page = () => {
	...
	const listServices = [
		...
		},
		{
			...
			task: "Our portfolio includes projects from government institutions and non-profit organizations, reflecting our versatility and commitment to excellence.",
		},
	];
	return (
		<div>
			...
			<div className="flex sm:flex-row flex-col-reverse">
				<div className="sm:w-1/3 bg-[#BEC6CF] xl:px-11 lg:px-9 md:px-7 px-6 xl:py-20 lg:py-16 md:py-12 py-6 flex flex-col gap-8">
					{listServicesLeft.map((data) => (
						<div className="flex items-start gap-4" key={data.title}>
							...
							/>
							<div>
								<h4 className="ff-outfit font-medium xl:text-[20px] lg:text-[18px] md:text-[16px] text-dark mb-4">
									{data.title}
								</h4>
								<p className="ff-poppins font-normal xl:text-[17px] text-[15px] text-dark-fade">
									{data.teks}
								</p>
							</div>
						</div>
				...
				<div className="sm:w-2/3 flex flex-col items-center justify-between ">
					<div className="max-w-[80%] xl:pt-24 lg:pt-16 md:pt-12 sm:pt-9 pt-6 xl:pb-32 lg:pb-24 md:pb-20 sm:pb-16 pb-12">
						<div className=" mb-12 ff-poppins font-bold text-[#2A2A2A] mx-auto bg-white xxl:text-[40px] xl:text-[36px] lg:text-[32px] md:text-[28px] text-[24px]">
							Get To Know Our{" "}
							<span
					...
					<div className="bg-[#2565AA] w-full py-9 ps-9 pr-14 flex md:flex-row flex-col md:items-center md:justify-between md:gap-2 sm:gap-6 gap-4 text-white">
						<h4 className="ff-outfit font-bold xl:text-[30px] lg:text-[26px] md:text-[22px] sm:text-[18px] text-[20px]">
							Get Free Consultation
						</h4>
						<div
							className="lg:px-[22px] md:px-[18px] sm:px-[14px] md:py-[17px] sm:py-[15px] py-[18px] ff-poppins md:text-[14px] sm:text-[12px] text-[16px] text-center font-semibold cursor-pointer transition-all duration-300 text-white hover:text-[#132e4c] bg-transparent hover:bg-white"
							style={{
			...
			<div
				className="py-12 text-white xl:px-28 lg:px-20 md:px-16 sm:px-14 px-8  grid md:grid-cols-3 sm:grid-cols-2 xl:gap-x-20 lg:gap-x-14 xl:gap-y-12 gap-y-10 "
				style={{
				...
				{listServices?.map((data, id) => (
					...
					>
						...
						<div>
							...
							<p className="ff-poppins text-[15px]">
